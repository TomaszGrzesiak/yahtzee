...
const of_a_kind_slot = (count: number): OfAKindSlot => ({
  type: "kind",
  count,
});
...
function of_a_kind(count: number, roll: number[]): number {
  const candidates = [...die_values]
    .reverse()
    .filter((target: number) => count_dice(target, roll) >= count);
...
function two_kinds(count1: number, count2: number, roll: number[]): number {
  ...
  const on_bottom = of_a_kind(
    count2,
    roll.filter((d) => d !== on_top)
  );
...
export function score(slot: Slot, roll: number[]): number {
  switch (slot.type) {
    ...
    case "small straight":
      return roll.toSorted().every((d: number, i: number) => d === i + 1)
        ? 15
        : 0;
    case "large straight":
      return roll.toSorted().every((d: number, i: number) => d === i + 2)
        ? 20
        : 0;
    case "full house":
