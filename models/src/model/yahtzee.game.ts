...
export function new_yahtzee({
  players,
  number_of_players,
  randomizer = standardRandomizer,
}: Readonly<YahtzeeOptions>): Yahtzee {
...
export function register(
  ...
  if (isLowerSection(slot)) {
    ...
    return {
      ...yahtzee,
      lower_sections: update(
        playerInTurn,
        register_lower(section, slot, roll),
        lower_sections
      ),
    ...
    return {
      ...yahtzee,
      upper_sections: update(
        playerInTurn,
        register_upper(section, slot, roll),
        upper_sections
      ),
      playerInTurn: (playerInTurn + 1) % yahtzee.players.length,
      roll: yahtzee.roller.roll(5),
...
export function is_finished(yahtzee: Omit<Yahtzee, "roller">): boolean {
  return (
    yahtzee.upper_sections.every(finished_upper) &&
    yahtzee.lower_sections.every(finished_lower)
  );
}
